...
const navigation = [
  ...
  { name: 'Uranus', element: <Link to="/uranus" /> },
  { name: 'Move to another planet', element: <Link to="/neptune" /> },
]

export default function Uranus() {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
  const { t } = useLanguage()
  const copy = t.planets.pages.uranus
  ...
  return (
    <div className="bg-black min-h-screen">
      ...
      >
        <source src="/assets/uranus.mp4" type="video/mp4" />
      </video>
          ...
          <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
            ...
            <div className="flex lg:flex-1">
              <a href="#" className="-m-1.5 p-1.5">
                <span className="sr-only">Uranus</span>
                <img
                  alt=""
        ...
        <div className="relative isolate px-6 pt-14 lg:px-8 min-h-screen flex items-center">
          <div className="max-w-xl lg:max-w-2xl lg:ml-24 py-32 sm:py-48 lg:py-56">
            <h1 className="text-5xl font-semibold tracking-tight text-white sm:text-7xl leading-tight">
              {copy.intro}{' '}
              <span className="text-cyan-300">Uranus</span>{' '}
              {' '}
            ...
            <div className="mt-10 flex items-center gap-x-6">
              <a
                href="#"
                className="rounded-md bg-cyan-300 px-4 py-2.5 text-sm font-semibold text-dark shadow hover:bg-cyan-200 transition-colors"
              >
                {copy.primaryAction}
              </a>
              <a
                href="#"
                className="text-sm/6 font-semibold text-white hover:text-cyan-300 transition-colors"
              >
